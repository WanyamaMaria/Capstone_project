...
<div class="container mx-auto px-4 py-6">
    ...
    <form action="{{ route('projects.update', $project) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        @csrf
        @method('PUT')
        ...
        <div class="mb-4">
            ...
            <textarea name="project_overview" id="project_overview"
        ...
        <div class="mb-4">
            ...
            <select name="nature_of_project" id="nature_of_project"
                ...
                @foreach(['Research', 'Prototype', 'Applied Work'] as $nature)
                    <option value="{{ $nature }}" {{ old('nature_of_project', $project->nature_of_project) == $nature ? 'selected' : '' }}>
                        {{ $nature }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <input type="text" name="innovation_focus" id="innovation_focus"
        ...
        <div class="mb-4">
            ...
            <select name="prototype_stage" id="prototype_stage"
                ...
                @foreach(['Concept', 'Prototype', 'MVP', 'Market Launch'] as $stage)
                    <option value="{{ $stage }}" {{ old('prototype_stage', $project->prototype_stage) == $stage ? 'selected' : '' }}>
                        {{ $stage }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <textarea name="testing_requirements" id="testing_requirements" rows="2"
                      class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">{{ old('testing_requirements', $project->testing_requirements) }}</textarea>
        ...
        <div class="mb-4">
            ...
            <textarea name="commercialization_plan" id="commercialization_plan" rows="2"
                      class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">{{ old('commercialization_plan', $project->commercialization_plan) }}</textarea>
        ...
        <div class="mb-4">
            ...
            <textarea name="description" id="description" rows="3"
                    class="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white rounded px-3 py-2">{{ old('description', $project->description) }}</textarea>
            @error('description')
                <div class="text-red-600 text-sm">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-4">
            ...
            <select name="facility_id" id="facility_id"
                ...
                @foreach($facilities as $facility)
                    <option value="{{ $facility->facility_id }}" {{ old('facility_id', $project->facility_id) == $facility->facility_id ? 'selected' : '' }}>
                        {{ $facility->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-4">
            ...
            <select name="program_id" id="program_id"
                ...
                @foreach($programs as $program)
                    <option value="{{ $program->program_id }}" {{ old('program_id', $project->program_id) == $program->program_id ? 'selected' : '' }}>
                        {{ $program->name }}
                    </option>
                @endforeach
            </select>
